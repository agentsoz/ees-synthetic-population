...
def derive_start_time (n_activity,durations):
...
def prob_matrix (n_activities,durations,repeating,type):
...
def plan_times (number_of_agents,probability_matrix,durations,repeating):
...
#%%
def location_map (locations_from_csv,location_names,subgroup):
...
#%%
def allocation_checker(activity_locations,number_of_agents,subgroup):
...
#%%
def plan_locations (plan_times,activity_locations,locality_distances,stay_in_locality):
...
#%%
def type_plan (n_activities,number_of_agents,travel_factor,locations_from_csv,location_names,type):
...
#%%
def write_log (AGENTS,DISTRIBUTIONS):
...
def write_xml (PLANS,output_location):
...
#%%
def read_distributions (distributions_file):

  df=pandas.read_csv(distributions_file,sep=',',skip_blank_lines= True)
  DISTRIBUTIONS=[]
  activities=[]
  # NUMBER=vector()
  # LOCATIONS=list()
...
#%%
def read_locations (s_file,types):
...
#%%
def read_numbers (numbers_file,types):
...
#%%
def read_locations_from_csv(locations_csv_file):
...
#%%
def inputs (distributions_file,locations_file,numbers_file,travel_file):
...
def main ()
  ...
  args=sys.argv
  args=["typical-summer-weekday/distributions.csv","typical-summer-weekday/location_maps.csv","typical-summer-weekday/numbers.csv","typical-summer-weekday/travel_factor.csv","Locations.csv","typical-summer-weekday/plans.xml"]

  input=inputs(distributions_file = args[0],locations_file = args[1],numbers_file = args[2],travel_file=args[3])  
